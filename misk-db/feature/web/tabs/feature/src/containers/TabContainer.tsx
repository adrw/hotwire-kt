...
const TabContainer = (props: IState & IDispatchProps) => {
  const query =
    ...
    "?"
  const path = ((props.location && props.location.pathname) || "").replace(
    "/api/dashboard/tab/misk-web/",
    "/_admin/"
  )
  const pathQueryEncoded =
    (props.location && props.location.pathname && `path=${path}`) || ""
